...
function preview(text, target)
{
	...
	{
		var nodes = resultFragment['querySelectorAll']('[data-akmaljp-livepreview-postprocess]'),
			i     = nodes.length;
		while (--i >= 0)
		{
			/** @type {!string} */
			var code = nodes[i]['getAttribute']('data-akmaljp-livepreview-postprocess');
	...
	*/
	function syncElementAttributes(oldEl, newEl)
	{
		var oldAttributes = oldEl['attributes'],
			...
			i             = oldCnt,
			ignoreAttrs   = ' ' + oldAttributes['data-akmaljp-livepreview-ignore-attrs'] + ' ';
